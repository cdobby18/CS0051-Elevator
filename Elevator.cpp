...
int main() {
    ...
    do {
        ...
        if (elevatorFloor == currentFloor) {
            cout << "You are already on Floor " << currentFloor << " using Elevator " 
                 << selectedElevator << ". Please choose a different floor." << endl;
        } else {
            thread t(simulateElevator, ref(currentFloor), elevatorFloor, selectedElevator);
            t.join();  // Wait for the elevator to finish moving
        }

        cout << "You have reached Floor Level #" << currentFloor << " using Elevator " 
             << selectedElevator << ". Do you want to use the elevator again? (Y/N): ";
        cin >> userChoice;
